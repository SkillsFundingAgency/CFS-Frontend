...
export function SpecificationFundingApproval({
  match,
}: RouteComponentProps<SpecificationFundingApprovalRouteProps>) {
  ...
  const hasPermissionToRefresh = useMemo(
  ...
  const loadingTitle = isLoadingSpecification
    ...
    : isLoadingRefresh
    ? "Requesting refresh of funding..."
    : isCheckingForJob
    ? "Checking for jobs..."
    : latestJob && latestJob.isActive
    ? `Job ${latestJob.statusDescription}: ${latestJob.jobDescription}`
    : isLoadingSearchResults
    ? "Loading provider funding data..."
    : isLoadingFundingConfiguration
    ? "Loading funding configuration..."
    : "";
  const loadingSubtitle = isLoadingRefresh
            ...
            <div className="govuk-grid-column-two-thirds">
              ...
              {specification?.fundingStreams?.length > 0 && specification?.fundingPeriod?.name && (
                <span className="govuk-caption-l" data-testid="fundingDetails">
                  {specification.fundingStreams[0].name} for{" "}
              ...
              <ul className="govuk-list right-align">
                ...
                <li>
                  <button
                    ...
                    disabled={
                      (latestJob && latestJob.isActive) || !hasPermissionToRefresh || isLoadingRefresh
                    }
