...
export function ConfirmDatasetToEdit({match}: RouteComponentProps<{relationshipId: string, specificationId: string}>) {
    ...
    const [relationshipDescription, setRelationshipDescription] = useState<string | undefined>(settings?.relationshipDescription);
    ...
    async function updateDatasetRelationship(): Promise<boolean> {
        if (!settings ||
            ...
            !settings.selectedItems) return false;
        try {
            const request = {
                ...
                fundingLineIds: settings.selectedItems?.filter(i => i.type === TemplateItemType.FundingLine).map(i => i.templateId),
                calculationIds: settings.selectedItems?.filter(i => i.type === TemplateItemType.Calculation).map(i => i.templateId),
            } as UpdateDatasetSpecificationRelationshipRequest;
            const response = await datasetService.updateDatasetFromReleased(request);
            return true;
        } catch (e) {
            addError({
                error: e,
    ...
    const onSave = async (e: React.FormEvent<HTMLFormElement> | React.MouseEvent<HTMLButtonElement>) => {
        ...
        if (success) {
            history.push(`/ViewSpecification/${settings.relationshipMetadata.currentSpecificationId}?showDatasets=true`)
        }
    };
    ...
    return (
        <Main location={Section.Datasets}>
            ...
            <Breadcrumbs>
                ...
                <Breadcrumb name={settings?.relationshipMetadata?.currentSpecificationName || "Specification"}
            ...
            <section>
                <Form token="confirm-dataset-changes"
                ...
                >
                    ...
                    <DatasetTemplateItemSelections
                        items={settings.selectedItems}
                    />
                    }
                    <Actions
                        relationshipId={relationshipId}
                        forSpecId={updatingSpecId}
                        onSave={onSave}
                    />
                </Form>
            </section>
...
const RelationshipDetails = (props: {
    ...
    return (<div className="govuk-grid-row govuk-!-margin-bottom-4">
...
const DatasetTemplateItemSelections = (props: {
...
const Actions = (props: {
    ...
    <div className="govuk-grid-row">
        <div className="govuk-grid-column-two-thirds">
            <h2 className="govuk-heading-m">
                Now update your data set
            </h2>
            <p className="govuk-body">
                By updating the data set the above funding line and calculations will be available for reference from this specification in your calculation scripts. 
            </p>
            <p className="govuk-body">
                Those removed will no longer be available
            </p>
            <div className="govuk-button-group">
                ...
                <Link to={`/Datasets/${props.relationshipId}/Edit/${props.forSpecId}`}
