...
export function ConfirmDatasetToCreate({match}: RouteComponentProps<CreateDatasetRouteProps>) {
    ...
    async function createDataset(model: CreateDatasetWorkflowState): Promise<boolean> {
        if (!model || 
            ...
            !model.selectedItems) return false;
        try {
            const request = {
                ...
                fundingLineIds: model.selectedItems?.filter(i => i.type === TemplateItemType.FundingLine).map(i => i.templateId),
                calculationIds: model.selectedItems?.filter(i => i.type === TemplateItemType.Calculation).map(i => i.templateId),
    ...
    const onSave = async (e: React.FormEvent<HTMLFormElement> | React.MouseEvent<HTMLButtonElement>) => {
        ...
        if (!state?.createDatasetWorkflowState || 
    ...
    return (
        <Main location={Section.Datasets}>
            ...
            <Breadcrumbs>
                ...
                <Breadcrumb name="Data set details"
                            url={`/Datasets/Create/SpecifyDatasetDetails/${forSpecId}`}/>
            ...
            <section>
                ...
                >
                    ...
                    <DatasetTemplateItemSelections
                        items={state.createDatasetWorkflowState?.selectedItems}
                    />
                    }
                    <Actions
                        forSpecId={forSpecId}
            ...
            <div className="govuk-summary-list__row govuk-!-width-one-third">
                ...
                <dd className="govuk-summary-list__actions">
                    <Link className="govuk-link"
                          to={`/Datasets/Create/SpecifyDatasetDetails/${props.forSpecId}`}>
...
const Actions = (props: {
    ...
    <div className="govuk-grid-row">
        <div className="govuk-grid-column-two-thirds">
            <h2 className="govuk-heading-m">
                Now create your data set
            </h2>
            <p className="govuk-body">
                By creating the data set the above funding line and calculations will be available
                for reference from this specification in your calculation scripts.
            </p>
            <div className="govuk-button-group">
                ...
                <Link to={`/Datasets/Create/SelectDatasetTemplateItems/${props.forSpecId}`}
