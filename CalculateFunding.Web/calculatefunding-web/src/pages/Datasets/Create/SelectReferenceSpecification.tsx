...
export function SelectReferenceSpecification({
  ...
  const { specification: forSpec } = useSpecificationSummary(forSpecId, (err) =>
    addError({
      error: err,
      description: "Error while loading specification",
    })
  );
  const { data: validOptions } = useQuery<EligibleSpecificationReferenceModel[], AxiosError>(
    `eligible-spec-ref-${forSpecId}`,
  ...
  const selectedSpec: EligibleSpecificationReferenceModel | undefined =
    validOptions &&
    selectedFundingStream &&
  ...
  const FundingStreamSelection = () => {
    ...
    return (
      ...
      >
        <label id="funding-stream-label" className="govuk-label" htmlFor="funding-stream">
          Funding stream
        </label>
        {errors.filter((e) => e.fieldName === "funding-stream").length > 0 &&
          errors.map(
            (error) =>
              error.fieldName === "funding-stream" && (
  ...
  const FundingPeriodSelection = () => {
    ...
    return (
      ...
      >
        <label id="funding-period-label" className="govuk-label" htmlFor="funding-period">
          Funding period
        </label>
        {errors.filter((e) => e.fieldName === "funding-period").length > 0 &&
          errors.map(
            (error) =>
              error.fieldName === "funding-period" && (
                <span key={error.id} className="govuk-error-message govuk-!-margin-bottom-1">
                  <span className="govuk-visually-hidden">Error:</span> {error.message}
                </span>
              )
        ...
        {selectedSpec ? (
          <>
            <div className="govuk-grid-column-two-thirds">
              <dl
                id="showSpecification"
                className="govuk-summary-list govuk-summary-list__row--no-border govuk-!-margin-top-5"
              >
                <div className="govuk-summary-list__row">
                  <dt id="term-specification-name" className="govuk-summary-list__key">
                    Specification
                  </dt>
                  <dd aria-labelledby="term-specification-name" className="govuk-summary-list__value">
    ...
    <div className="govuk-grid-row">
      <div className="govuk-grid-column-two-thirds">
        <div className="govuk-inset-text">
          There are no funding streams to select. There is either no data sharing enabled with the funding
          stream of the current specification or there are no enabled funding streams with released data.
        </div>
        <BackLink />
      </div>
    </div>
          ...
          <div>
            {validOptions ? (
              <>
                ...
                {validOptions.length === 0 && <NoOptions />}
              </>
            ) : (
