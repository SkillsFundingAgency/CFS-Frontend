...
export function FundingStreamPermissionsAdmin() {
    ...
    const fundingStreams: FundingStream[] = useMemo(() => permissions && permissions
        .filter(fs => fs.canAdministerFundingStream)
        .map(fs => ({
            id: fs.fundingStreamId,
            name: fs.fundingStreamName
        } as FundingStream)), [permissions]);
            ...
            <section className="govuk-grid-row govuk-!-margin-top-2">
                <div className="govuk-grid-column-two-thirds">
                    <form id="download-form"
                        ...
                        <div className="govuk-form-group">
                            ...
                            <select id="fundingStream"
                                ...
                                {fundingStreams
                                    ...
                                    .map(fs =>
                                        <option value={fs.id} key={fs.id}>{fs.name}</option>
                                    )}
                            </select>
                        </div>
                        ...
                        {loading &&
                        <LoadingFieldStatus title={"Generating report of users with permissions against this funding stream"}/>
                        }
                        {!loading && report?.url &&
                        <ConfirmationPanel title="Report generated successfully">
                            <a href={report.url}
                               className="govuk-panel__link"
                               target="_self">
                                Download report
                            </a>
                        </ConfirmationPanel>
