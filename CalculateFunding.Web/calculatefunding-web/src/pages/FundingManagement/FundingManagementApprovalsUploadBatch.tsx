...
export function FundingManagementApprovalsUploadBatch({ match }: RouteComponentProps<UploadBatchRouteProps>) {
    ...
    const { mutate: uploadBatchFile, isLoading: isUploadingBatchFile } = useMutation<BatchUploadResponse,
        AxiosError,
        File>(
        async (theFile) => {
    ...
    useEffect(() => {
        ...
        if (latestJob.isFailed) {
            ...
            history.push(
                `/FundingManagementApprovalsConfirmFunding/${fundingStreamId}/${fundingPeriodId}/${specificationId}/${actionType}`,
            ...
            {isBlocked && (
                <LoadingStatusNotifier notifications={[
                    ...
                    },
                    {
                        ...
                        subTitle: latestJob && latestJob.isActive
