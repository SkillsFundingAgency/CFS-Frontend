...
import {
  ...
  getMultipleVersionsByCalculationIdService,
...
export function CompareCalculationVersions({
  match,
}: RouteComponentProps<CompareCalculationVersionsRouteProps>) {
  ...
  function populateCalculation(calculationId: string) {
    getCalculationByIdService(calculationId)
      .then((result) => {
  ...
  function populateCalculationVersions(calculationId: string, versions: number[]) {
    getMultipleVersionsByCalculationIdService(calculationId, versions)
      .then((result) => {
        ...
        <div className="govuk-grid-row">
          <div className="govuk-grid-column-full">
            <p className="govuk-body">
              Funding Period: <span>{specification.fundingPeriod.name}</span>
            </p>
          ...
          <div className="govuk-grid-column-one-half">
            <div className="govuk-form-group">
              <label className="govuk-label" htmlFor="more-detail">
                <DateTimeFormatter date={calculationVersions[0].lastUpdated} />{" "}
                <span className="right-align">{calculationVersions[0].publishStatus}</span>
              </label>
            </div>
          </div>
          <div className="govuk-grid-column-one-half">
            <div className="govuk-form-group">
              <label className="govuk-label" htmlFor="more-detail">
                <DateTimeFormatter date={calculationVersions[1].lastUpdated} />{" "}
                <span className="right-align">{calculationVersions[1].publishStatus}</span>
              </label>
            </div>
          </div>
          <div className="govuk-grid-column-full">
            <div className="govuk-form-group">
              <GdsMonacoDiffEditor
                firstCalculationVersion={calculationVersions[0].sourceCode}
                secondCalculationVersion={calculationVersions[1].sourceCode}
                inlineCodeViewer={inlineCodeView}
              />
            </div>
          </div>
        </div>
                ...
                <div className="govuk-checkboxes govuk-checkboxes--small">
                  <div className="govuk-checkboxes__item">
                    <input
                      ...
                      name="organisation"
                      type="checkbox"
                      onChange={(e) => changeInlineCodeView(e)}
                    />
