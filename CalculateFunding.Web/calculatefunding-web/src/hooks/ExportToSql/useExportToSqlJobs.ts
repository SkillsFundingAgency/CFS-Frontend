...
export const useExportToSqlJobs = ({
        ...
        () => {
            ...
            jobNotifications.filter((n) =>
                ((n.latestJob?.jobType === JobType.ReleaseProvidersToChannelsJob) ||
                    ...
                    (n.latestJob?.jobType === JobType.PublishBatchProviderFundingJob)) && n.latestJob?.isSuccessful);

            const job = jobs.sort(n => DateTime.fromJSDate(n.latestJob?.lastUpdated ?? new Date(0)).toMillis()).shift();
    ...
    const isReleasedAllocationDataInSqlOutdated =
        ...
        (!!latestPublishedDate?.value && (lastReleasedAllocationExportJob?.lastUpdated !== undefined ? latestPublishedDate.value > lastReleasedAllocationExportJob?.lastUpdated : true));
    ...
    return {
        ...
        isLatestAllocationStateBlockedByJob: isExportingReleasedResults || hasRunningReleasedAllocationSqlImportJob,
        ...
        isLatestReleaseDataAlreadyExported: hasPreviousReleasedAllocationSqlExport && !hasPreviousReleasedAllocationSqlExportFailure &&
