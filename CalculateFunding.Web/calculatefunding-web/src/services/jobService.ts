...
export async function getJobStatusUpdatesForSpecification(
...
export async function getLatestJobByJobDefinitionId(
  jobDefinitionId: JobType
): Promise<AxiosResponse<JobResponse | undefined>> {
  return axios(`${baseURL}/latest-by-job-definition-ids/${jobDefinitionId}`, {
    method: "GET",
    headers: { "Content-Type": "application/json" },
  });
}
...
export async function getLatestJobByEntityId(
  specificationId: string,
  entityId: string
): Promise<AxiosResponse<JobResponse | undefined>> {
