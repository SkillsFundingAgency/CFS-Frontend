...
import {
  clonedCalculationsAndFundingLinesNodeDs,
  clonedFundingLinesDs,
  clonedNodeDs,
  clonedNodeTemplate,
  cloneWithChildrenNodeDs,
  multipleCalculationsDs,
  multipleFundingLinesDs,
  multipleFundingLinesTemplate,
  singleNodeDs,
  singleNodeTemplate,
  withChildFundingLineAndCalculationDs,
  withChildFundingLineAndCalculationTemplate,
  withChildFundingLineDs,
  withChildFundingLineTemplate,
...
it("can delete clone if it's a clone root but only deletes that clone instance (child calc was cloned)", async () => {
  ...
  const clonedNodeToDelete = cloneDs
    .find((d) => d.key === 2)
    ?.value.children?.find((c) => c.id === "n0:12345");
...
it("updates clones when new child node is moved onto original", async () => {
  ...
  expect(
    cloneDs.find((d) => d.key === 1)?.value.children?.find((c) => c.id === "n0")?.children?.length
  ).toEqual(1);
  ...
  expect(
    cloneDs.find((d) => d.key === 2)?.value.children?.find((c) => c.id === "n0:12345")?.children?.length
  ).toEqual(2);
});
...
it.skip("transforms cloned node template into datasource", () => {
...
it("calculates getAllCalculations correctly", () => {
  ...
  expect(getAllCalculations(withChildFundingLineAndCalculationDs.map((d) => d.value))).toEqual([
    { id: "n2", name: "Calculation 3", templateCalculationId: 3, aggregationType: "None" },
  ]);
  expect(getAllCalculations(multipleFundingLinesDs.map((d) => d.value))).toEqual([
    { id: "n2", name: "Calculation 4", templateCalculationId: 4, aggregationType: "Sum" },
  ]);
  expect(getAllCalculations(clonedNodeDs.map((d) => d.value))).toEqual([
  ...
  expect(
    getAllCalculations(
      clonedCalculationsAndFundingLinesNodeDs.map((d) => d.value),
      true
    )
  ).toEqual([
    { id: "n0", name: "Calculation 4", templateCalculationId: 4, aggregationType: "Sum" },
...
it("calculates parentId correctly", () => {
  expect(
    findParentId(
      singleNodeDs.map((d) => d.value),
      "n0"
    )
  ).toEqual("");
  expect(
    findParentId(
      withChildFundingLineDs.map((d) => d.value),
      "n0"
    )
  ).toEqual("n1");
  expect(
  ...
  expect(
    findParentId(
      withChildFundingLineAndCalculationDs.map((d) => d.value),
      "n0"
    )
  ).toEqual("n1");
  expect(
  ...
  expect(
    findParentId(
      multipleFundingLinesDs.map((d) => d.value),
      "n0"
    )
  ).toEqual("");
  expect(
  ...
  expect(
    findParentId(
      clonedNodeDs.map((d) => d.value),
      "n0:12345"
    )
  ).toEqual("n3");
  expect(
    findParentId(
      multipleCalculationsDs.map((d) => d.value),
      "n3"
    )
  ).toEqual("n4");
  expect(
    findParentId(
      cloneWithChildrenNodeDs.map((d) => d.value),
      "n0:12345"
    )
  ).toEqual("n3");
});
