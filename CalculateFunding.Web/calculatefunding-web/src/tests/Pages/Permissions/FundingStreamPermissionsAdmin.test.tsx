...
describe("<FundingStreamPermissionsAdmin />", () => {
  describe("given I am an admin user for a funding stream ", () => {
    ...
    it("renders page title", async () => {
      ...
      expect(
        await within(title).findByRole("heading", { name: /Funding stream permissions/ })
      ).toBeInTheDocument();
      expect(
        await within(title).findByText(/Download all user permissions for a funding stream/)
      ).toBeInTheDocument();
    });
...
const mockUserService = () => {
  jest.mock("../../../services/userService", () => {
    ...
    return {
      ...service,
      getReportOnUsersByFundingStream: jest.fn(() =>
        Promise.resolve({
          status: 200,
          data: mockReport,
        })
...
const renderPage = async () => {
  const {
    FundingStreamPermissionsAdmin,
  } = require("../../../pages/Permissions/FundingStreamPermissionsAdmin");
  return render(
    <MemoryRouter initialEntries={["/Permissions/FundingStream"]}>
