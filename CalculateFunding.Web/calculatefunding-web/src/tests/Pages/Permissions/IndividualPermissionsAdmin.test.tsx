...
describe("<IndividualPermissionsAdmin/>", () => {
  describe("given I am an admin user for a funding stream ", () => {
    ...
    it("renders page title", async () => {
      ...
      expect(
        await within(title).findByRole("heading", { name: /Set and view user permissions/ })
      ).toBeInTheDocument();
      expect(
        await within(title).findByText(/Assign and view permissions for users of Calculate funding service/)
      ).toBeInTheDocument();
    });

    it("renders section heading", async () => {
      expect(
        await screen.findByRole("heading", { name: /Set and view user permissions/ })
      ).toBeInTheDocument();
    });
    ...
    describe("when I select a user", () => {
      ...
      describe("and when I select a funding stream and click Continue", () => {
        ...
        it("renders new description text", async () => {
          expect(
            await screen.findByText(/Assign and view permissions for user 537 for Lalala funding stream/)
          ).toBeInTheDocument();
        });
        ...
        it("renders the user's permissions correctly", async () => {
          ...
          allCheckboxes.forEach((checkbox) => {
            if (checkbox.title === Permission.CanCreateTemplates.toString()) expect(checkbox).toBeChecked();
        ...
        describe("and when I change permissions and click to save it", () => {
          beforeEach(async () => {
            const checkbox1 = screen.getByRole("checkbox", {
              name: Permission.CanCreateTemplates.toString(),
            });
        ...
        describe("and when I change permissions, save and then navigate away", () => {
          beforeEach(async () => {
            const checkbox1 = screen.getByRole("checkbox", {
              name: Permission.CanEditCalculations.toString(),
            });
            expect(checkbox1).toBeInTheDocument();
            expect(checkbox1).not.toBeChecked();
            userEvent.click(checkbox1);
        ...
        describe("and when I click to remove all of a user's permissions", () => {
          ...
          it("calls the api correctly to save the updated permissions and displays notification", async () => {
            ...
            expect(
              within(modal).getByText(/Are you sure you want to remove all user permissions for/)
            ).toBeInTheDocument();
            expect(within(modal).getByText(/user 537/)).toBeInTheDocument();
            expect(
              within(modal).getByText(/and delete them from the Lalala funding stream?/)
            ).toBeInTheDocument();
...
const mockUserService = () => {
  jest.mock("../../../services/userService", () => {
    ...
    return {
      ...
      getOtherUsersPermissionsForFundingStream: jest.fn(() =>
        Promise.resolve({
          status: 200,
          data: mockUserPermissions,
        })
      ),
      updateOtherUsersPermissionsForFundingStream: jest.fn(() =>
        Promise.resolve({
          status: 200,
          data: mockUserPermissions,
        })
      ),
      removeOtherUserFromFundingStream: jest.fn(() =>
        Promise.resolve({
          status: 200,
        })
      ),
...
const renderPage = async () => {
  ...
  return render(
    <MemoryRouter initialEntries={["/Permissions/Individual"]}>
