  ...
  describe("when user has canApplyCustomProfilePattern permission", () => {
    ...
    describe("when existing profile pattern has already been set", () => {
      ...
      it("saves new rule based profile", async () => {
        ...
        await waitFor(() => {
          ...
          expect(assignProfilePatternKeyToPublishedProvider).toBeCalledWith(
            "DSG",
            "FY-2021",
            "10005143",
            "DSG-004",
    ...
    describe("when only national pattern exists", () => {
      ...
      it("displays no rule based patterns message", async () => {
        ...
        expect(
          screen.getByText(
            /No rule based patterns are available. Pupil Led Factors is using the national pattern./
          )
        ).toBeInTheDocument();
...
const mockPreviewProfile = jest.fn(() =>
  Promise.resolve({
    data: [],
  })
);
...
jest.mock("../../../services/providerService", () => ({
  getProviderByIdAndVersionService: jest.fn(() =>
    Promise.resolve({
      data: {
