...
function renderComponent() {
  ...
  return render(
    <MemoryRouter initialEntries={["/ViewResults/ViewProvidersByFundingStream/DSG"]}>
      <Switch>
        <Route
...
function mockPolicyService() {
  ...
  return {
    ...policyService,
    getFundingStreamByIdService: jest.fn(() =>
      Promise.resolve({
...
function mockProviderService() {
  ...
  return {
    ...providerService,
    getProvidersByFundingStreamService: jest.fn(() =>
      Promise.resolve({
        data: {
          totalCount: 1234,
          startItemNumber: 1,
...
describe("<ViewProvidersByFundingStream /> ", () => {
  ...
  it("has the View results breadcrumb", async () => {
    const { container } = renderComponent();
    await waitFor(() =>
      expect(container.querySelectorAll(".govuk-breadcrumbs__list-item")[1].textContent).toBe("View results")
    );
  });
