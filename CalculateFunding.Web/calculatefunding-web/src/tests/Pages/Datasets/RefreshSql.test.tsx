...
describe("<ChangeProfileType /> ", () => {
  ...
  describe("when no previous sql job exists", () => {
    ...
    it("shows sql job status panel when push data button clicked", async () => {
      ...
      expect(
        screen.getByText(/Please do not refresh the page, you will be redirected automatically/i)
      ).toBeInTheDocument();
      await act(() => promise);
    });
    ...
    it("does not show sql job status panel when push data button clicked when endpoint returns server error", async () => {
      ...
      await waitForElementToBeRemoved(
        screen.queryByText(/Please do not refresh the page, you will be redirected automatically/i)
      );
  ...
  describe("when a previous sql job exists", () => {
    ...
    it("shows previous job run date and shows warning message if job already run", async () => {
      ...
      expect(
        screen.getByText(/Refresh SQL data is not available as the latest version has already been pushed/i)
      ).toBeInTheDocument();
...
jest.mock("../../../services/specificationService", () => ({
  getSpecificationsSelectedForFundingService: jest.fn(() =>
    Promise.resolve({
      data: [
        {
          id: "DSG",
