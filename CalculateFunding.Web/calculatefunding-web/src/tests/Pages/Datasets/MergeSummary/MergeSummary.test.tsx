...
describe("<MergeSummary />", () => {
  ...
  it(" displays the correct data schema name", async () => {
    const { container } = renderMergeSummary(1, 1, "Test DataSchema Name", 1, "Test Data Source Name", false);
    await waitFor(() =>
      expect(container.querySelector("#data-schema")?.textContent).toBe("Test DataSchema Name")
    );
  });
  ...
  it(" displays the correct data source name", async () => {
    const { container } = renderMergeSummary(1, 1, "Test DataSchema Name", 1, "Test Data Source Name", false);
    await waitFor(() =>
      expect(container.querySelector("#data-source")?.textContent).toBe("Test Data Source Name")
    );
  });
});
