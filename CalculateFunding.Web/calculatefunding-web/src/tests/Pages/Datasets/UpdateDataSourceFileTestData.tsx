...
export function UpdateDataSourceFileTestData() {
  ...
  const mockValidateDatasetService = jest.fn(() =>
    Promise.resolve({
      status: 200,
      data: {
  ...
  const mockValidateDatasetServiceWithBadResult = jest.fn(() =>
    Promise.reject({
      status: 400,
      response: {
  ...
  const mockGetDatasetHistoryService = jest.fn(() =>
    Promise.resolve({
      data: {
        results: [
          {
  ...
  const mockUpdateDatasetService = jest.fn(() =>
    Promise.resolve({
      status: 200,
      data: {
        blobUrl:
          "https://strgt1dvcfsv2.blob.core.windows.net/datasets/b6135804-128a-4863-8f47-e6b9143ef0ca/v2/Book1.xlsx?sv=2019-07-07&sr=b&sig=WA68s76hz6DCCdku8amnF3LxsVXSHyg6q8Qzsjiq76I%3D&se=2020-11-21T11%3A46%3A04Z&sp=rw",
  ...
  const mockUploadDatasetVersionService = jest.fn(() =>
    Promise.resolve({
      status: 200,
      data: { mockUpdateNewDatasetVersionResponseViewModel },
    })
  );
  const mockDownloadValidateDatasetValidationErrorSasUrl = jest.fn(() =>
    Promise.resolve({
      status: 200,
      data: "aTestValidationReportUrl",
    })
  );
  const mockGetCurrentDatasetVersionByDatasetId = jest.fn(() =>
    Promise.resolve({
      data: {
        blobName: "a/very/long/blob/url/with-an-excelfile-at-the-end.xlsx",
        version: 1,
  ...
  const mockProviderService = () => {
    jest.mock("../../../services/providerService", () => {
      ...
      return {
        ...service,
        getCurrentProviderVersionForFundingStream: jest.fn(() =>
          Promise.resolve({
            data: {
              name: "",
              providerVersionId: 1,
  ...
  const renderPage = async () => {
    ...
    const page = render(
      <MemoryRouter initialEntries={["/Datasets/UpdateDataSourceFile/DSG/DATASET123"]}>
        <Switch>
          <Route
