...
describe("<LoadNewDataSource />", () => {
  ...
  describe("renders elements on initial page load ", () => {
    ...
    it("will find the description Load a new data source file to create a dataset to use in calculations.", async () => {
      expect(
        screen.getByText("Load a new data source file to create a dataset to use in calculations.")
      ).toBeInTheDocument();
    });
  });
...
const mockPolicyService = () => {
  jest.mock("../../../services/policyService", () => {
    ...
    return {
      ...service,
      getFundingStreamsService: jest.fn(() =>
        Promise.resolve({
          data: [
            {
              shortName: "1619",
...
const mockDatasetService = () => {
  jest.mock("../../../services/datasetService", () => {
    ...
    return {
      ...
      getDatasetDefinitionsService: jest.fn(() =>
        Promise.resolve({
          data: [
            {
              description: "Early Years AP Census Data Year 1",
      ...
      getDatasetHistoryService: jest.fn(() =>
        Promise.resolve({
          data: {
            results: [
              {
      ...
      uploadDataSourceService: jest.fn(() =>
        Promise.resolve({
          status: 200,
        })
      ),
      validateDatasetService: jest.fn(() =>
        Promise.resolve({
          status: 200,
          data: {
      ...
      downloadValidateDatasetValidationErrorSasUrl: jest.fn(() =>
        Promise.resolve({
          status: 200,
          data: "aTestValidationReportUrl",
        })
      ),
      getCurrentDatasetVersionByDatasetId: jest.fn(() =>
        Promise.resolve({
          data: {
            blobName: "a/very/long/blob/url/with-an-excelfile-at-the-end.xlsx",
            version: 1,
