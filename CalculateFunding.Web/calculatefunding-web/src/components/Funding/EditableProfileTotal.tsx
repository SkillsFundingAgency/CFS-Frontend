...
export function EditableProfileTotal({
  ...
  useEffect(() => {
    ...
    const formattedPercent = formatNumber(
      profileTotal.profileRemainingPercentage ? profileTotal.profileRemainingPercentage : 0,
      NumberType.FormattedPercentage,
      7,
      false
    );
    ...
    <>
      <td className="govuk-table__cell" data-testid={`remaining-percentage-${index}`}>
        {!isEditMode || isPaid ? (
          ...
          <div
            className={`govuk-form-group editable-field ${
          ...
          >
            {errors.filter((e) => e.fieldName === `percent-${installmentNumber}`).length > 0
              ? errors.map(
                  (error) =>
                    error.fieldName === `percent-${installmentNumber}` && (
            ...
            <div className="govuk-input__wrapper">
              <input
                ...
                autoComplete="off"
              />
              <div className="govuk-input__suffix" aria-hidden="true" aria-label="Enter amount in percent">
                %
              </div>
            </div>
          </div>
        ...
        {!isEditMode || isPaid ? (
          ...
          >
            {errors.filter((e) => e.fieldName === `value-${installmentNumber}`).length > 0
              ? errors.map(
                  (error) =>
                    error.fieldName === `value-${installmentNumber}` && (
                      <span key={error.id} className="govuk-error-message govuk-!-margin-bottom-1">
                        <span className="govuk-visually-hidden">Error:</span> {error.message}
                      </span>
                    )
            ...
            <div className="govuk-input__wrapper">
              <div
                className="govuk-input__prefix right-align"
