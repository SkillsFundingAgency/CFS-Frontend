...
const SpecificationDataRelationshipsGrid = React.memo(({
    ...
    return (
        <table id="datarelationship-table"
               data-testid={"datarelationship-table"}
...
const DataSetDetails = ({
    ...
    return (
        <details className="govuk-details govuk-!-margin-bottom-0 govuk-!-margin-top-2"
            ...
            {item.relationshipType === DatasetRelationshipType.Uploaded ?
                <div className="govuk-details__text">
                    ...
                    <p className="govuk-body">
                        <strong>Last mapped:</strong> <DateTimeFormatter
...
const DataRelationshipItemRow = React.memo(({
    item,
    converterWizardJobs
}: DataRelationshipItemRowProps) => {

    const hasConverterWizardRunning = useMemo(() =>
            converterWizardJobs.some(n =>
                n.latestJob?.triggeredByEntityId === item.relationshipId &&
                n.latestJob?.isActive),
        [item.relationshipId, converterWizardJobs]);

    return (
        <tr className="govuk-table__row"
            data-testid={`tr-relationship-${convertToSlug(item.relationshipId)}`}
        >
