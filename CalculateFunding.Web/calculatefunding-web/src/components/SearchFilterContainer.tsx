...
export const SearchFilterSectionHeader = (
  {filterName, handleToggleExpand, selectedFilters, isExpanded}:
    {
      filterName: string,
  ...
  return (
    <div className="search-filters__section__content"
         data-testid={id}>
      <h2 className="search-filters__section-heading">
        <label className="search-filters__section-label govuk-body govuk-!-font-size-19"
               htmlFor={id}
               onClick={handleToggleExpand}>
        <span className="search-filters__section-heading-text">
          <span className="search-filters__section-heading-text-focus"
                id={id}>
          ...
          <span className="search-filters__section-toggle-focus">
            <span className={`search-filters-nav__chevron search-filters-nav__chevron--${!isExpanded ? "up" : "down"}`}>
              &nbsp;
            </span>
...
export const SearchFilterSelection = (
  {title, facets, searchForFilter, selectedFilters, addFilter, removeFilter}:
    {
      title: string,
      facets: FacetValue[],
    ...
    <div className="search-filters__section__content">
      <fieldset className="govuk-fieldset">
        <div className="govuk-form-group filterSearch">
          <label id={`label-search-${id}`}
                 className="govuk-label filterLabel"
                 htmlFor={`search-${id}`}>
...
export const SearchFilterSection = React.memo((
  {title, facets, searchForFilter, selectedFilters, isExpanded, toggleExpanded, addFilter, removeFilter}:
    {
      title: string,
      facets: FacetValue[],
      isExpanded: boolean,
  ...
  return (
    <div className="search-filters__section">
      <SearchFilterSectionHeader filterName={title}
                                 handleToggleExpand={toggleExpanded}
                                 isExpanded={isExpanded}
                                 selectedFilters={selectedFilters}
      />
      {isExpanded && <SearchFilterSelection title={title}
                                            selectedFilters={selectedFilters}
...
export const TextSearchPanel = ({
  ...
  return (
    <div className="govuk-form-group filterbyContainer">
      <fieldset className="govuk-fieldset">
        <div className="govuk-form-group filterSearch">
          <label className="govuk-heading-s govuk-!-display-inline-block govuk-!-margin-top-2 govuk-!-margin-bottom-2"
                 htmlFor="mainContentSearch">
            {title}
          </label>

          <TextLink handleOnClick={handleClearSearch}
...
export const SelectedFilters = ({title, selectedFilters, handleRemoveFilter}:
                                  {
                                    title: string;
                                    selectedFilters: string[]
    ...
    <div className="filter">
      <div className="filter__content">
        <div className="filter__selected">
          <h3 className="govuk-hint govuk-body govuk-!-font-size-16 govuk-!-margin-0">
            {title}
          </h3>
...
export const SearchFilterSelectionPanel = ({
  ...
  const haveFilters = !!selectedFundingStreamFilters.length ||
          ...
          (
            <div className="filtersSelected">
              {selectedFundingStreamFilters.length > 0 &&
                  <SelectedFilters title="Funding streams"
...
export const SearchFiltersOuterContainer = React.memo((
  {expandAllFilters, collapseAllFilters, allExpanded, allCollapsed, children}:
    {
      ...
      <div className="filterContainer">
        <div className="search-filters__controls">
          <label onClick={(e) => handleClick(e)}
            ...
            <span
              className={`search-filters-nav__chevron search-filters-nav__chevron--${!allExpanded ? "up" : "down"}`}>
              &nbsp;
            </span>
            <span id="show-hide-all"
                  className="search-filters__show-all-text">
...
export const SearchFilterContainer = ({children}: { children: any }) => {
  return (
    <div className="govuk-form-group filterContainer">
      {children}
    </div>
  );
}
  ...
  return (
    <div className="search-filters__controls">
      <label
        ...
        className="search-filters__show-all" aria-expanded={isExpanded}
        htmlFor={id}
      >
        <span className={`search-filters-nav__chevron search-filters-nav__chevron--${isExpanded ? "up" : "down"}`}>
        </span>
        <span className="search-filters__show-all-text">
          {isExpanded ? expandedTitle : collapsedTitle}
        </span>
      </label>
      {isExpanded &&
          <section id={id} aria-labelledby={`label-${id}`} className="search-filters__section">{children}</section>}
    </div>);
};
