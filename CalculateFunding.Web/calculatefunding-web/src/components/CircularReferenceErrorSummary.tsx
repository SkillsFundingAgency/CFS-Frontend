...
export const CircularReferenceErrorSummary = ({
  errors,
  defaultSize,
}: CircularReferenceErrorSummaryProps) => {
  ...
  return (
    <div
      className="govuk-error-summary circular-reference-error"
      ...
      <div className="govuk-error-summary__body">
        ...
        <ul className="govuk-list govuk-error-summary__list">
          {errors.slice(0, defaultSize).map((e) => (
            <li key={e.node.calculationid}>{createLink(e)}</li>
          ))}
          {errors.length > defaultSize && !showMore ? (
            <li>
              <button className="govuk-link" onClick={handleShowMore}>
                Show more
              </button>
            </li>
          ) : null}
          {showMore &&
            errors.slice(defaultSize).map((e) => <li key={e.node.calculationid}>{createLink(e)}</li>)}
        </ul>
      </div>
    </div>
  );
