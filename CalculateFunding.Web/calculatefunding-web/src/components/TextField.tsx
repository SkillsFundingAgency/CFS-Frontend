...
export const TextField = ({ token, label, hint, value, isLoading, errors, onChange }: TextFieldProps) => {
  const hasErrors = !!errors?.length;

  return (
    <div className={`govuk-form-group ${hasErrors ? "govuk-form-group--error" : ""}`}>
      <label
        className="govuk-label govuk-!-margin-bottom-2"
        id={`text-${token}-label`}
        htmlFor={`text-${token}`}
      >
        {label}
      ...
      {!isLoading && (
        <>
          {hint?.length && (
            <div>
              <span id={`text-${token}-hint`} className="govuk-hint govuk-!-margin-bottom-2">
                {hint}
              </span>
            </div>
          )}
