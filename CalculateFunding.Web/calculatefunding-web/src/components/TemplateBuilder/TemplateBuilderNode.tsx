      ...
      <div>
        ...
        {isEditMode && !isClone && (
          <div className="buttons">
            <button className="govuk-button" onClick={handleAddLine} data-testid={`${node.id}-add-line`}>
              Add Line
            </button>
            <button className="govuk-button" onClick={handleAddCalc} data-testid={`${node.id}-add-calc`}>
              Add Calculation
            </button>
          </div>
        )}
  ...
  return (
    <div>
      <div onClick={handleClick}>
        ...
        <div className="box govuk-body-s">
          {node.valueFormat !== undefined ? node.type + " (" + node.valueFormat + ")" : node.type}
        </div>
      </div>
      {isEditMode && !isClone && (
        <div className="buttons">
          <button className="govuk-button" onClick={handleAddCalc}>
            Add Calculation
          </button>
        </div>
      )}
