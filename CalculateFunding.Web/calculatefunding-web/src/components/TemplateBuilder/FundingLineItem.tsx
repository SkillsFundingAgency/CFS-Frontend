            ...
            <fieldset className="govuk-fieldset" key={node.id}>
                <div className={`govuk-form-group ${isNameValid ? "" : "govuk-form-group--error"}`}>
                    ...
                    {isEditMode ?
                        ...
                        :
                        <span id="fl-name" className="govuk-hint">{name}</span>
                    }
                </div>
                <div className={`govuk-form-group ${!isTemplateLineIdValid ? "govuk-form-group--error" : ""}`}>
                    ...
                    {isEditMode ?
                        <>
                            {!isTemplateLineIdValid && 
                                <span id="template-line-id-error" className="govuk-error-message">
                                    <span className="govuk-visually-hidden">Error:</span>
                                    {templateLineId.length > 0 ? 'This funding line ID is already in use.' : 'Funding line ID is required.'}
                                </span>
                            }
                            <input className="govuk-input" id="fl-template-line-id" name="fl-template-line-id" type="text" 
                                   value={templateLineId} onChange={handleTemplateLineIdChange}/>
                        </>
                        :
                        <span id="template-line-id" className="govuk-hint">{templateLineId}</span>
                    }
                </div>
                <div className="govuk-form-group">
                    ...
                    {isEditMode ?
                        ...
                        :
                        <span id="fl-type" className="govuk-hint">{type}</span>
                    }
                </div>
                ...
                <div className="govuk-form-group">
                    ...
                    {isEditMode ?
                        <>
                            ...
                            <input className="govuk-input govuk-!-width-two-thirds" id="fl-code" name="fl-code" type="text" value={code}
                                   onChange={handleCodeChange}/>
                        </>
                        :
                        <span id="fl-code" className="govuk-hint">{code}</span>
                    }
                </div>
                }
                {isEditMode ?
