          ...
          >
            ...
            {fundingStreams && (
              <select
                ...
                id="fundingStreamId"
                data-testid="fundingStreamId"
                name="fundingStreamId"
                onChange={handleFundingStreamChange}
              >
                {sortedFundingStreams.map((stream) => (
                  <option key={stream.id} value={stream.id} data-testid={"fundingStreamOption"}>
                    {stream.name}
                  </option>
                ))}
              </select>
            ...
            {errors.map(
              (error) =>
                error.fieldName === "fundingStreamId" && (
      ...
      <div className="govuk-grid-column-full">
        {fundingStreams && fundingStreams.length > 0 && fundingPeriods && (
          <div
            className={
              "govuk-form-group " +
              (errors.some((error) => error.fieldName === "fundingPeriodId") ? "govuk-form-group--error" : "")
            }
          >
            ...
            {errors.map(
              (error) =>
                error.fieldName === "fundingPeriodId" && (
                  <span key={error.id} className="govuk-error-message">
                    <span className="govuk-visually-hidden">Error:</span> {error.message}
                  </span>
                )
