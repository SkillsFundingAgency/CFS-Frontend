...
export function CollapsibleSteps(props: React.PropsWithChildren<ICollapsibleStepsProps>) {
  ...
  const hasError: boolean =
    props.calculationErrorMessage === undefined || props.calculationErrorMessage === null
      ? false
      : props.calculationErrorMessage.length > 0;
      ...
      <li ref={props.customRef} key={"step" + listKey} className="collapsible-step step-is-shown">
        ...
        >
          <h2 className={props.step === "1" ? "govuk-heading-s first-step-title" : "govuk-heading-s"}>
            <span
              className="collapsible-step-circle collapsible-step-circle-number"
              hidden={props.step === ""}
            >
              <span className="collapsible-step-circle-inner">
                <span className="collapsible-step-circle-background">
                  ...
                  {props.step}
                  <span className="collapsible-step-circle-step-colon visuallyhidden" aria-hidden="true">
                    :
                  </span>
                </span>
              </span>
            </span>
            <span className="collapsible-step-header-title">{props.title}</span>
            <span
              className={`collapsible-step-header-description 
                            ${hasError ? "govuk-form-group--error" : ""}`}
            >
              {props.link !== "" ? (
                <Link to={props.link} className="govuk-link">
                  {props.description}
                </Link>
              ) : (
            ...
            <span
              className="collapsible-step-panel-button"
              hidden={!props.hasChildren}
              onClick={updateExpandedStatus}
            >
              <label
                className={
                  expanded
        ...
        <div
          key={listKey + "panel"}
          className={
            expanded ? "collapsible-step-panel" : "collapsible-step-panel collapsible-step-hidden-panel"
          }
        >
          {props.children}
        </div>
...
export function setCollapsibleStepsAllStepsStatus(fundingLines: FundingStructureItemViewModel[]) {
  ...
  fundingLines.map(function searchFundingLines(fundingStructureItem: FundingStructureItemViewModel) {
