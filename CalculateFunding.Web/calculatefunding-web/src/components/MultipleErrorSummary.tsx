          ...
          <ul className="govuk-list govuk-error-summary__list">
            {props.errors.map((error, errIdx) => (
              <li key={errIdx}>
                ...
                {error.fieldName && (
                  <a href={"#" + error.fieldName} className="govuk-link govuk-link-red">
                    {error.message}
                  </a>
                )}
                ...
                {error.validationErrors && (
                  <ul className="govuk-list">
                    {Object.keys(error.validationErrors).map((errKey, valErrKeyIdx) =>
                      errKey === "blobUrl" ? (
                        <BlobValidationError
                        ...
                        <li key={`${errIdx}-${valErrKeyIdx}`}>
                          {error.validationErrors &&
                            error.validationErrors[errKey].map((err, errIdx) =>
                              err.includes("#VariationInstallmentLink#") ? (
                                ...
                                <BasicValidationError key={`validation-error-${errIdx}`} error={err} />
                              )
                            )}
                        </li>
                      )
...
const BlobValidationError = (props: { errorFields: ValidationErrors | undefined }) => {
  return (
    <li>
      ...
      <a href={props.errorFields && props.errorFields["blobUrl"]?.toString()}>error report</a>
    </li>
  );
};
...
const VariationInstallerValidationError = (props: { error: string; specificationId: string | undefined }) => {
  ...
  return (
    <>
      <span>
        {variationSplitter[0]}{" "}
        <Link to={`/ViewSpecification/${props.specificationId}`}>Variation Installment Link</Link>{" "}
        {variationSplitter[1]}
      </span>
      <br />
    </>
