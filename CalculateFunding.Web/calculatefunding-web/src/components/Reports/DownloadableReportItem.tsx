...
export function DownloadableReportItem(props: DownloadableReportItemProps) {
    ...
    return <div key={`grouplevel-${props.itemKey}`}
        ...
        <div className="attachment__details"
             key={`attachment-details-${props.itemKey}`}>
            <h4 className="govuk-heading-s">
                <a className="govuk-link" target="_self"
                   href={`/api/specs/${props.reportMetadataViewModel.specificationReportIdentifier}/download-report`}>{props.reportMetadataViewModel.name}</a>
            </h4>
            <p className="govuk-body-s">
                <span>{props.reportMetadataViewModel.format}</span>, <span>{props.reportMetadataViewModel.size}</span>,
                Updated: <span><DateTimeFormatter
                date={props.reportMetadataViewModel.lastModified}/></span>
            </p>
        </div>
    </div>
}