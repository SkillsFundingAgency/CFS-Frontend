...
export function MappingStatus({jobMessage}: MappingStatusProps) {
    ...
    if (jobMessage.runningStatus === RunningStatus.Queued) {
        cssClass += " govuk-error-summary-orange";
        title = "Mapping queued"
    }

    if (jobMessage.runningStatus === RunningStatus.InProgress) {
        cssClass += " govuk-error-summary-orange";
        title = "Mapping in progress"
    }

    if (jobMessage.runningStatus === RunningStatus.Completed) {
        completed = true;
        if (jobMessage.completionStatus === CompletionStatus.Succeeded) {
    ...
    const jobCreatedDateTime = () => {
        ...
        if (isJobSummary) {
            ...
            return <DateFormatter date={job.created as Date} utc={false} />
        }
    ...
    const jobStatusDateTime = () => {
        ...
        if (isJobSummary) {
            job = jobMessage as JobSummary;
            return <DateFormatter date={job.lastUpdated as Date} utc={false} />
        }
        job = jobMessage as JobMessage;
