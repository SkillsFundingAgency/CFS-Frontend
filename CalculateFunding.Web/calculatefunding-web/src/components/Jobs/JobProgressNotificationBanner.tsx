...
export function JobProgressNotificationBanner(props: JobProgressNotificationBannerProps) {
  const title = useMemo(() => {
    ...
    if (props.job.isActive && props.jobInProgressOverride) return props.jobInProgressOverride.heading;

    if (props.job.isFailed && props.jobFailedOverride) return props.jobFailedOverride.heading;

    if (props.job.isSuccessful && props.jobSuccessfulOverride) return props.jobSuccessfulOverride.heading;

    return `Job ${props.job.statusDescription}: ${props.job.jobDescription}${
      props.job.outcome && props.job.outcome.length > 0 ? ": " + props.job.outcome : ""
    }`;
  ...
  const description = useMemo(() => {
    ...
    if (props.job.isActive && props.jobInProgressOverride) return props.jobInProgressOverride.description;

    if (props.job.isFailed && props.jobFailedOverride) return props.jobFailedOverride.description;

    if (props.job.isSuccessful && props.jobSuccessfulOverride) return props.jobSuccessfulOverride.description;
  ...
  const cssClass = useMemo(() => {
    ...
    return props.job.isFailed
      ? "govuk-error-summary-red"
      : props.job.isSuccessful
  ...
  return (
    ...
    >
      <h2 className="govuk-error-summary__title" id="error-summary-title">
        ...
        {props.job.isActive && <div className="loader loader-small" role="alert" />}
