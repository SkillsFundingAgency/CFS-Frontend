...
export function CalculationSourceCode(props: CalculationSourceCodeProps) {
  ...
  useEffect(() => {
    if (state.isBuilding) {
      ...
      compileCalculationPreviewService(
        ...
        props.calculationId ?? "temp-calc-id",
        previewCompileRequestViewModel
      )
        .then((result) => {
          setState((prevState: CalculationSourceCodeState) => {
            return {
              ...prevState,
              calculationBuild: {
                hasCodeBuiltSuccessfully:
                  result.status === 200 && result.data?.compilerOutput?.success === true,
                previewResponse: result.data,
                isProviderValid:
                  state.providerId !== "" &&
                  result.data?.previewProviderCalculation?.providerName !== undefined &&
                  result.data?.previewProviderCalculation?.providerName !== "",
                providerRuntimeException:
                  result.data?.previewProviderCalculation?.calculationResult?.exceptionMessage,
  ...
  return (
    <div
      id="source-code"
      className={
        "govuk-form-group govuk-!-margin-bottom-0 govuk-!-margin-top-3" +
        (state.calculationBuild.hasCodeBuiltSuccessfully === false ? " govuk-form-group--error" : "")
      }
    >
      <h4 className="govuk-heading-s">Calculation script</h4>
      ...
      {state.calculationBuild.hasCodeBuiltSuccessfully !== undefined && (
        <div
          className={
            "govuk-inset-text" +
            (!state.calculationBuild.hasCodeBuiltSuccessfully ? " govuk-form-group--error" : "")
          }
        >
          <label className="govuk-label">
            <h4 className="govuk-heading-s">Build output</h4>
          </label>
            ...
            <div>
              ...
              {!state.calculationBuild.isProviderValid && (
                <p className="govuk-body">
                  <strong>No provider found. Try a different UKPRN.</strong>
                </p>
              )}
              {state.calculationBuild.isProviderValid &&
                state.calculationBuild.providerRuntimeException !== "" && (
                  <p className="govuk-body">
                    <strong>{state.calculationBuild.providerRuntimeException}</strong>
                  </p>
                )}
              {state.calculationBuild.providerName !== "" && (
                <p className="govuk-body">
                  <strong>Provider: {state.calculationBuild.providerName}</strong>
                </p>
              )}
              {state.calculationBuild.providerResult !== undefined && (
                <p className="govuk-body">
                  <strong>Provider result: {state.calculationBuild.providerResult.value}</strong>
                </p>
              )}
            </div>
          )}
