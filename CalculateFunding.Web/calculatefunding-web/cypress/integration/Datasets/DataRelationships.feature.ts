...
context('Given a specification with data relationships', () => {
    ...
    describe('when page has loaded', () => {
        it('displays the specification name', () => {
            cy.findByRole('heading', {name: dataRelationshipResponse.specification.name})
                .should('exist');
        });
        it('displays the funding period name', () => {
            cy.findByText(dataRelationshipResponse.specification.fundingPeriod.name)
                .should('exist');
        });
    ...
    describe('when I click to add a new data set', () => {
        ...
        it('goes to the correct page', () => {
            cy.findByRole('link', {name: /Add new data set/i})
                .click()
                .url()
                .should('include', '/Datasets/CreateDataset/' + specification.id);
        });
    });
