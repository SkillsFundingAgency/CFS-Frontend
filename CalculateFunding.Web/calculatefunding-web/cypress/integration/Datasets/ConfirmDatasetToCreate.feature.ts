...
context("Confirm details for creating a new dataset from released", () => {
  ...
  describe("when page has loaded", () => {
    it("has correct title", () => {
      cy.findByRole("heading", { name: /Check funding lines\/calculations before creating data set/ }).should(
        "exist"
      );
    });
    it("has link to spec", () => {
    ...
    it("has dataset name", () => {
      cy.findByText(/Dataset123/).should("exist");
    });
    it("has dataset description", () => {
      cy.findByText(/lorem ipsum description/).should("exist");
    });
    it("has template item name", () => {
      cy.findByText(/Calculate total pupil mass/).should("exist");
    });
    it("has Create button", () => {
      cy.findByRole("button", { name: /Create data set/ }).should("exist");
    });
  });

  describe("when user clicks Create, it redirects to View Spec page", () => {
    it.skip("goes to correct page", () => {
      cy.findByRole("button", { name: /Create data set/ }).click();
